...
class Mappa {
    ...
    Mappa(int forma, int n_city, Random& rnd) {
        double appo {};
        m_ncity = n_city; 

        if(forma == 0) {
            for(int i=0; i<n_city; i++) {
                appo = 2 * M_PI * rnd.Rannyu();
                m_coord.push_back(cos(appo));
                m_coord.push_back(sin(appo));
            }
        }

        else{
            for(int i=0; i<n_city; i++) {
                m_coord.push_back(rnd.Rannyu(-1, 1));
                m_coord.push_back(rnd.Rannyu(-1, 1));
            }
        }

        //Una volta generati i punti scelgo di calcolare le distanze fra le varie città (in modo tale da non farlo un tot di volte)
        //Ottengo una matrice di dimensione n_city * n_city, mi basta calcolare metà degli elementi perchè matrice simmetrica
    ...
    vector<vector<double>> Get_costo() { return m_costo; }
    ...
    void Stampa_coord(string nome) {
        ...
        if(!fileout) {   cerr << "Problema in apertura file: " + nome << endl; }
...
class Cammino {
    ...
    Cammino(Random& rnd, Mappa& mappa) {
        ...
        //Riempio il vettore con sequenza ordinata da 0 a 33; 
    ...
    void Scambia_seq(int iniz, int len, int dist) {
        ...
        if(len >= (m_lengh - dist)/2) {
            cerr << "Sequenza selezionata è troppo lunga!!" << endl;
            cerr << "Non scambio la sequenza" << endl;
        }
    ...
    void Sposta_seq(int iniz, int len, int dist) {
        ...
        if(len >= (m_lengh - 1)) {
            cerr << "Sequenza selezionata è troppo lunga!!" << endl;
            cerr << "Non sposto la sequenza" << endl;
        }
    ...
    void Inverti(int iniz, int len) {
        ...
        if(len > (m_lengh - 1)) {
            cerr << "Sequenza selezionata è troppo lunga!!" << endl;
            cerr << "Non eseguo inversione" << endl;
        }
...
class TravelingSalesman{
    ...
    TravelingSalesman() {;}
    //Costruttore con input
    TravelingSalesman(Random& rnd, int ncity, int forma, int ncamm) {
        m_forma = forma; //Forma scelta
        m_ncamm = ncamm; //Numero di cammini

        //Lavoro con la mappa
        Mappa disp(forma, ncity, rnd);
        m_disp = disp;
        m_ncity = ncity;

        popo.resize(m_ncamm);
        for(int i=0; i<ncamm; i++) { //Inizializzo la popolazione
            Cammino test(rnd, m_disp);
            popo[i] = test;
        }
        popo[0].Stampa_coord("cir_config.out.dat", m_disp);
    ...
    void GA(int n_gen, Random& rnd) {
        ...
        if(m_forma == 0) {
            fileout.open("best_circ.out.dat");
            file_out.open("mean_circ.out.dat");
        }

        else{
            fileout.open("best_sq.out.dat");
            file_out.open("mean_sq.out.dat");
        }
        ...
        if(!file_out || !fileout) {
            ...
            for(int i=0; i<n_gen; i++) {
                ...
                for(int j=0; j<m_ncamm/2; j++) {
                    ...
                    Mutazione(m_ncamm + 2*j, rnd); Mutazione(m_ncamm + 2*j + 1, rnd); //Provo a farli mutare
                ...
                fileout << i << "   " << m_best << endl;
                file_out << i << "   " << m_av << endl;
            }
        }
    ...
    void StampaBest() {
        if(m_forma == 0) { popo[0].Stampa_coord("best_cir_config.out.dat", m_disp); }
        else { popo[0].Stampa_coord("best_sq_config.out.dat", m_disp); }
    }
